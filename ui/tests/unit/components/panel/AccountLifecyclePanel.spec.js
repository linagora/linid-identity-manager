...
import AccountLifecyclePanel from '../../../../src/components/panel/AccountLifecyclePanel.vue';
import {
  mockToDate,
  mockToDateISO,
  mockToDayJs,
 ...
 *
 * @param entity - The raw account entity provided by the hosting page.
 * @returns The shallow-mounted wrapper.
 */
function mountPanel(entity = accountDto) {
  return shallowMount(AccountLifecyclePanel, {
    props: {
      entity,
      uiNamespace: 'moduleAccountDetailsPage',
    },
  });
}
...
describe('Test component: AccountLifecyclePanel', () => {
  ...
  describe('Test computed: lifecycleUi', () => {
    ...
    it('should expose the projected lifecycle UI once the entity is resolved', () => {
      wrapper = mountPanel();
  ...
  describe('Test function: updateAccountStatus', () => {
    ...
    it('should emit the account-status-updated uiEvent with the updated DTO on success', async () => {
      const updatedDto = { ...accountDto, status: 'SUSPENDED' };
  ...
  describe('Test functions: lifecycle action dialogs', () => {
    ...
    beforeEach(() => {
      ...
      mockedSuspendAccount.mockResolvedValue(accountDto);
      mockedDeactivateAccount.mockResolvedValue(accountDto);
      mockedReactivateAccount.mockResolvedValue(accountDto);
      mockedSetAccountValidity.mockResolvedValue(accountDto);
      wrapper = mountPanel();
      mockUiEventSubjectNext.mockClear();
