Feature: Test Static List from Mock API
  ...
  ## 101 Should display role fields as StaticList on creation page
  ...
  Scenario: Complete static list workflow for role field
    ...
    ## 201 Should display required validation error message
    When I visit the '{{ env.E2E_FRONT_URL }}/moduleUsers/new'
    And I click on '[data-cy="field_roleWithInvalidDefaultValue"]'
    And I click on '[data-cy="title"]'
    Then I expect the HTML element '[data-cy="field-container_roleWithInvalidDefaultValue"] .q-field__messages' to be visible
