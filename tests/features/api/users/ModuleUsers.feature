Feature: Test API Users Module
  ...
  ## TODO: Enable theses tests when patch are supported
  ...
  Scenario: 502 - Should return 400 for invalid JSON body on PUT
    When I set http header 'Content-Type' with 'application/json'
    And I request '{{env.E2E_API_URL}}/api/users/any-user-id' with method 'PUT' with body:
      """
      { invalid json }
      """
    Then I expect status code is 400
