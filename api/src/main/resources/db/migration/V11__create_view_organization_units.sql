...
COMMENT ON VIEW organizational_units_view IS 'Aggregated view exposing organizational units along with their parent organizational units and their suspension status (suspension period, reason, sub-reason, comment and a computed is_suspended flag).';
...
COMMENT ON COLUMN organizational_units_view.status_reason IS 'High-level reason code explaining the current suspension status.';
COMMENT ON COLUMN organizational_units_view.status_subreason IS 'More detailed classification of the status reason.';
COMMENT ON COLUMN organizational_units_view.status_comment IS 'Free-text comment providing additional context about the status change.';
