 ...
 * preference scoped to a user (identified by its id). Inherits audit fields from
...
public class UserPreference extends AbstractEntity {
     ...
     *
     * <p>Unique per user: the pair ({@code userId}, {@code key}) is constrained to be unique,
     * so the same key may exist for different users but only once per user.</p>
     */
    @Column(name = "usp_key", nullable = false)
     ...
     */
    @Column(name = "usp_value", nullable = false)
    @FilterType(type = String.class)
