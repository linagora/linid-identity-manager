Feature: Tests API Users Preferences endpoints
  ...
  Background:
    ...
    And I setup database with driver "postgres" host "localhost" port 5432 user "admin" password "password" database "linid-db"
    And I execute sql request "DELETE FROM user_preferences"
  ...
  Scenario: 202 - Should return 404 when deleting an unknown key
    When  I request '{{env.E2E_API_URL}}/user-preferences' with method 'POST' with body:
      ...
      """
    Then I expect status code is 201
    And  I expect '{{response.body.value}}' is 'x'

    When  I request '{{env.E2E_API_URL}}/user-preferences/unknown_key' with method 'DELETE'
    Then  I expect status code is 204
