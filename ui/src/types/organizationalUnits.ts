 ...
 * client-provided values, with no server-managed metadata.
 */
export interface OrganizationalUnitRecord {
  /**
   * Identifier of the parent organizational unit. Always required: the root
   * is the only OU without a parent and is created by the backend.
   */
  parent: string;
  /**
...
export interface OrganizationalUnitFilterDTO {
  /**
   * Name filter to apply when querying organizational units.
...
/**
 * Shape of the OU creation form. The `parent` value is provided by the
 * navigation context and is therefore not present here; the form only carries
