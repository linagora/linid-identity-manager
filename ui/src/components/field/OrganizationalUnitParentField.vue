...
import { type OrganizationalUnitParentFieldProps } from 'src/types/organizationalUnitParentFieldProps';
...
const parentUiInputProps = ui<LinidQInputProps>(
  `${props.uiNamespace}.organizational-unit-parent-field`,
  'q-input'
);

/**
 * Resolves the parent OU from the route query and keeps its name for display, emitting the resolved id on
 * {@link OrganizationalUnitParentFieldProps.emitOnParentLoaded}. Creating an OU without a parent is not allowed, so on
 * error the user is redirected to {@link OrganizationalUnitParentFieldProps.listPath}.
 */
async function loadParent(): Promise<void> {
  ...
  if (!id) {
    ...
    void router.push(props.listPath);
    return;
  }
  try {
