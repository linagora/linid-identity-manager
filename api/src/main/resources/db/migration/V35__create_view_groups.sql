...
COMMENT ON VIEW groups_view IS 'Read-only view exposing group records with the name of the parent group, the name of the associated organizational unit and application, and createdBy/updatedBy resolved to the full name (firstname + lastname) of the referenced account.';
...
COMMENT ON COLUMN groups_view.parent_name IS 'Name of the parent group. Resolved via LEFT OUTER JOIN on groups.grp_id; NULL when the group has no parent.';
...
COMMENT ON COLUMN groups_view.name IS 'Human-readable name of the group.';
